...
# Check this: https://nickymeuleman.netlify.app/garden/aoc2022-day16
...
while not finished['me'] or not finished['elephant']:

    order = sorted([(times[k], k) for k in times.keys()], reverse=False)
    # print('Another!')
    ...
    for _, name in order:
        ...
        options = [r for r in [r_map[k] for k in r_map.keys()] if r.name != node.name and not r.active]
        ...
        for o in options:
            ...
            elif time < 10 and score == 0:
                print(f'   ❌{o.name}: {score}, -{cost}s ({rate:.1f}/s) (fr: {o.flow_rate})')

        if len(possible_scores) > 0:
            possible_scores.sort(reverse=True)

            print(f'[{name.upper()}] From {node.name} ({(26-time)}s, Rate: {total_rate})')
            for r, n, s, c, fr in possible_scores:
                print(f'   {n}: {s}, -{c}s ({r:.1f}/s) (fr: {fr})')
            ...
            r_map[best[1]].active = True
            nodes[name] = r_map[best[1]]
            # print(f'Set {best[1]} active\n')

        else:
            print(f'[{name.upper()}] Finish time ({(26-time)}s, Rate: {total_rate})')

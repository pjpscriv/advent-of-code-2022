...
class Valley():
    ...
    def navigate(self):
        ...
        if (self.minutes > self.loop_time):
            ...
            first_loop_positions = self.positions[first_loop_minutes]
        ...
        for p in self.positions[last_round]:
            ...
            if self.end in new_positions:
                break

        new_positions = list(new_positions)

        # Filter out invalid positions
        # print(f"    pos check {new_positions}")
        new_positions = [
        ...
        # print(f"    bliz check {new_positions}")
        new_positions = [
            (x, y) for x, y in new_positions
        ...
        # print(f"    past check {new_positions}")
        new_positions = [
            (x, y) for x, y in new_positions
...
print()
valley.print_blizzards(True)

while end not in valley.positions[valley.minutes] and len(valley.positions[valley.minutes]) > 0:
    ...
    valley.print_blizzards(True)
    # print(f"{valley.positions[valley.minutes]}")

print(f"Done! Completed in minutes: {valley.minutes}")

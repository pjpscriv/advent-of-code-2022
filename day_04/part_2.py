class Pair():
    ...
    def overlap(self):
        x = self.e2[0] <= self.e1[0] and self.e1[0] <= self.e2[1]
        y = self.e2[0] <= self.e1[1] and self.e1[1] <= self.e2[1]
        return x or y
...
containings = [ (1 if (p.overlap() or p.fully_contained()) else 0) for p in pairs]
